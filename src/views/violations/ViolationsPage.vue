<template>
  <ion-page class="violations-page">
    <ion-content :fullscreen="true">
      ...
      <AppEmptyState
        v-else-if="filteredViolations.length === 0"
        icon="shield-outline"
      ...
      <div v-else class="list-container">
        ...
        >
          <div class="violation-card__top">
            <div class="violation-card__avatar">
              <span>{{ getInitials(item.studentName) }}</span>
            </div>

            <div class="violation-card__main">
              <div class="violation-card__heading">
                <span class="violation-card__student">{{ item.studentName }}</span>
                <AppStatusBadge :status="item.status" :label="STATUS_LABELS[item.status] || item.statusLabel" />
              </div>
              <span class="violation-card__type">{{ item.violationType }}</span>
              <span class="violation-card__class">{{ item.className || 'Kelas belum tersedia' }}</span>
              <p v-if="item.description" class="violation-card__description">{{ item.description }}</p>
            </div>
          </div>
          ...
          <div class="violation-card__footer">
            ...
            <div class="violation-card__meta" v-if="item.location">
              <ion-icon :icon="locationOutline" />
              <span>{{ item.location }}</span>
            </div>
            <div class="violation-card__points" v-if="item.point > 0">
...
import { alertCircleOutline, calendarOutline, chevronDownOutline, chevronUpOutline, filterOutline, locationOutline, refreshOutline } from 'ionicons/icons'
...
const normalizedViolations = computed(() => {
  return violations.value.map(item => {
    ...
    return {
      ...
      violationType: item.violation_type || item.type?.name || 'Menunggu penentuan kategori',
      className,
      status: String(item.status || 'pending').toLowerCase(),
      statusLabel: item.status || 'pending',
      date: item.date,
      location: item.location || '',
      point: Number(item.point || item.points || item.type?.point || 0),
...
const filteredViolations = computed(() => {
  return normalizedViolations.value.filter(item => {
...
const emptyDescription = computed(() => {
  if (!selectedClass.value && !selectedStudent.value && selectedDate.value === getTodayValue()) {
    return 'Belum ada catatan pelanggaran untuk hari ini.'
  }
...
async function fetchViolations() {
  try {
    const { data } = await mobileReportsApi.getMyViolations(1, 100)
...
async function loadPage() {
  loading.value = true
  await Promise.allSettled([fetchFilters(), fetchViolations()])
  loading.value = false
}
...
.violation-card__points ion-icon {
  font-size: 14px;
}
</style>
